...
class EigFromY:
    def __init__(self, ps, model):
        ...
        # Analytical (symbolic-parameterized) VSC TFs (per device)
    ...
    def build_Ybus_lines(self):
        ...
        # --- Voltage source contributions (as large admittance) ---
    ...
    # -------------------------------------------------------------------------
    # Analytical transfer function (multiple VSCs)
    # -------------------------------------------------------------------------
    ...
    def analytical_transfer_function(self, s):
        ...
        vsc_params = {
            'UIC_open': dict(Ki=0.05, xf=0.10),
            'UIC1': dict(Ki=0.01, xf=0.10),
            'UIC2': dict(Ki=0.02, xf=0.20),
        }
        ...
        for dev in self.vsc_devices:
            ...
            P = self.ps.vsc['UIC_open']._input_values['p_ref'][0]
            Q = self.ps.vsc['UIC_open']._input_values['q_ref'][0]
