...
class PowerSystemModelLinearization:
    ...
    def get_mode_idx(self, mode_type=None, damp_threshold=1, freq_range=None, sorted=True):
        # Get indices of modes from specified criteria.
        if mode_type is None:
            mode_type = ['em', 'non_conj']
...
if __name__ == '__main__':
    ...
    import casestudies.ps_data.dataTaskALS1 as model_data
    ...
    mode_idx = ps_lin.get_mode_idx(['E'], damp_threshold=0.9)
    rev = ps_lin.rev
    mode_shape = rev[np.ix_(ps.vsc['DROOP_VSC_GEN'].state_idx_global['omega'], mode_idx)]
